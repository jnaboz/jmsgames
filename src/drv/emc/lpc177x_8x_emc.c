...
/*
    ...
    PINSEL_SetSlewMode(2,16,1);
    //PINSEL_SetPinMode(2,16,IOCON_MODE_REPEATER);

    PINSEL_ConfigPin(2,17,1);
    PINSEL_SetSlewMode(2,17,1);
    //PINSEL_SetPinMode(2,17,IOCON_MODE_REPEATER); 

    PINSEL_ConfigPin(2,18,1);
    PINSEL_SetSlewMode(2,18,1);
    //PINSEL_SetPinMode(2,18,IOCON_MODE_REPEATER);
*/
				//PINSEL_ConfigPin(2,19,1);		/* Not Used */
/*
	PINSEL_ConfigPin(2,20,1);
    PINSEL_SetSlewMode(2,20,1);
    //PINSEL_SetPinMode(2,20,IOCON_MODE_REPEATER);
*/
...
/*
    ...
    PINSEL_SetSlewMode(2,24,1);
    //PINSEL_SetPinMode(2,24,IOCON_MODE_REPEATER);
*/
...
/*
    ...
    PINSEL_SetSlewMode(2,28, 1);
    //PINSEL_SetPinMode(2,28,IOCON_MODE_REPEATER);

    PINSEL_ConfigPin(2,29,1);
    PINSEL_SetSlewMode(2,29,1);
    //PINSEL_SetPinMode(2,29,IOCON_MODE_REPEATER); 
*/
...
/*
	for(i = 0; i < 16; i++)						/* D0 to D15 *
	{
        ...
        PINSEL_SetSlewMode(3,i,1);
        PINSEL_SetPinMode(3,i,IOCON_MODE_REPEATER); 
	}
	
	for(i = 0; i < 11; i++)						/* A0 to A10 *
	{
		PINSEL_ConfigPin(4,i,1);
        PINSEL_SetSlewMode(4,i,1);
        //PINSEL_SetPinMode(4,i,IOCON_MODE_REPEATER); 
	}

	PINSEL_ConfigPin(4,13,1);
    PINSEL_SetSlewMode(4,13,1);
    //PINSEL_SetPinMode(4,13,IOCON_MODE_REPEATER); 
*/
    

    /* Set up IO pins : CAS, RAS, CLK0, DYCS0, CKE0, DQM0, DQM1, WE */
    LPC_IOCON->P2_16 |= 1;
    LPC_IOCON->P2_17 |= 1;
    LPC_IOCON->P2_18 |= 1;
    LPC_IOCON->P2_20 |= 1;
    ...
    /* Set up Address pins : A0 .. A10, A13 (Bank select) */
    LPC_IOCON->P4_0  |= 1;
    LPC_IOCON->P4_1  |= 1;
    LPC_IOCON->P4_2  |= 1;
    LPC_IOCON->P4_3  |= 1;
