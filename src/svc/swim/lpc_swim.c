...
void swim_put_triangle(SWIM_WINDOW_T *win, U2 x0, U2 y0, U2 x1, U2 y1, U2 x2, U2 y2)
{
  U2 xtemp, ytemp;
  U4 dx_far;
  U4 dx_upper;
  ...
  if (y1 > y2)
    ...
    for (y = y0; y <= (y2 > height-1 ? height-1 : y2); y++)
    {
        if (y >= 0)
        {
            for (x = (xf > 0 ? xf : 0); x <= (xt < width ? xt : width-1) ; x++)
            {
                *FBXY(win, x, y) = win->fill;
            }
            for (x = (xf < width ? xf : width-1); x >= (xt > 0 ? xt : 0); x--)
            {
                *FBXY(win, x1, y1) = win->fill;
            }
        }
